...
import { Play, Pause, RefreshCw, Eye, EyeOff, Camera, Video, ShieldCheck, Cpu, MapPin, Gauge, ChevronDown, ChevronUp, Layers } from 'lucide-react';
import { SimulationScene, ViolationType } from '../types';
import { UrbanEyeIcon } from './logos';
...
export const HaydenSimulationCanvas: React.FC<HaydenSimulationCanvasProps> = ({
  ...
  const [cardTransparency, setCardTransparency] = useState<'translucent' | 'ultra' | 'solid'>('translucent');
    ...
    <div className="relative w-full rounded-2xl overflow-hidden border border-slate-200 bg-[#0F172A] shadow-md">
      ...
      <div className="absolute top-0 inset-x-0 z-20 flex flex-wrap items-center justify-between gap-3 px-5 py-3 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-blue-950/80 border border-blue-500/40 shadow-xs">
            <UrbanEyeIcon size={20} />
            <span className="text-[10px] font-black tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-sky-300">
              URBAN EYE
            </span>
          </div>
      ...
      <div ref={containerRef} className="w-full h-[460px] sm:h-[520px] relative">
        ...
        {/* Floating Hayden AI Style Card - Compact, Translucent Glassmorphic HUD overlay */}
        {isCardCollapsed ? (
          <div
            onClick={() => setIsCardCollapsed(false)}
            className="absolute bottom-12 sm:bottom-16 left-3 sm:left-6 z-20 flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-950/70 hover:bg-slate-900/90 backdrop-blur-md border border-white/20 text-white shadow-xl cursor-pointer transition-all text-xs"
            title="Click to expand details"
          >
            <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: currentScene.color }} />
            <span className="font-bold text-[11px] text-white truncate max-w-[160px] xs:max-w-[220px]">
              {currentScene.title}
            </span>
            <span className="text-[9px] text-emerald-400 font-mono hidden sm:inline">
              {currentScene.impactMetric.split(' ')[0]} impact
            </span>
            <ChevronUp className="w-3.5 h-3.5 text-slate-300 ml-0.5 shrink-0" />
          </div>
        ) : (
          <div
            className={`absolute bottom-12 sm:bottom-16 left-3 sm:left-6 z-20 w-[calc(100%-1.5rem)] max-w-[270px] xs:max-w-[300px] sm:max-w-xs md:max-w-sm p-3 sm:p-3.5 rounded-xl sm:rounded-2xl transition-all duration-200 border shadow-2xl text-white ${
              cardTransparency === 'ultra'
                ? 'bg-slate-950/40 hover:bg-slate-950/60 backdrop-blur-sm border-white/15'
                : cardTransparency === 'solid'
                ? 'bg-slate-900/92 backdrop-blur-xl border-slate-700/60'
                : 'bg-slate-950/60 hover:bg-slate-950/75 backdrop-blur-md border-white/15'
            }`}
          >
            {/* Header: Badges and Transparency / Minimize Controls */}
            <div className="flex items-center justify-between gap-1.5 mb-1.5">
              <div className="flex items-center gap-1.5 min-w-0">
                <span
                  className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border shadow-2xs shrink-0"
                  style={{
                    backgroundColor: `${currentScene.color}22`,
                    color: currentScene.color,
                    borderColor: `${currentScene.color}44`
                  }}
                >
                  {currentScene.badge}
                </span>
                <span className="text-[9px] sm:text-[10px] text-slate-300/80 font-mono truncate hidden xs:inline max-w-[110px] sm:max-w-[150px]">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    setCardTransparency((prev) =>
                      prev === 'translucent' ? 'ultra' : prev === 'ultra' ? 'solid' : 'translucent'
                    );
                  }}
                  className="px-1.5 py-0.5 rounded bg-white/10 hover:bg-white/20 text-[9px] font-mono text-slate-200 border border-white/10 transition-colors"
                  title="Toggle transparency: 40% Glass, 60% Glass, or Solid"
                >
                  {cardTransparency === 'ultra' ? '40% Glass' : cardTransparency === 'translucent' ? '60% Glass' : 'Solid'}
                </button>
                <button
                  type="button"
                  onClick={() => setIsCardCollapsed(true)}
                  className="p-1 rounded bg-white/10 hover:bg-white/20 text-slate-200 transition-colors"
                  title="Minimize card to reveal full background"
                >
            ...
            {/* Title */}
            <h3 className="text-xs xs:text-sm sm:text-base font-bold text-white tracking-tight leading-snug mb-1">
              {currentScene.title}
            </h3>

            {/* Description */}
            <p className="text-[10px] xs:text-[11px] sm:text-xs text-slate-200 leading-snug sm:leading-relaxed mb-2 line-clamp-2 xs:line-clamp-3 sm:line-clamp-none">
              {currentScene.description}
            </p>

            {/* Footer metrics & statutory penalty */}
            <div className="flex items-center justify-between gap-1.5 pt-2 border-t border-white/10 text-[10px] sm:text-xs">
              <div className="font-semibold text-emerald-400 flex items-center gap-1 truncate">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                <span className="truncate">{currentScene.impactMetric}</span>
              </div>
              {currentScene.fineInr > 0 && (
                <span className="shrink-0 font-bold text-amber-300 bg-amber-500/20 px-1.5 py-0.5 rounded border border-amber-500/30 font-mono text-[9px] sm:text-[10px]">
                  Penalty: ₹{currentScene.fineInr.toLocaleString()}
                </span>
              )}
            </div>
          </div>
